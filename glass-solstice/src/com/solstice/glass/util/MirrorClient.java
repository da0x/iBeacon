...
public class MirrorClient {
  ...
  public static Mirror getMirror(Credential credential) {
    return new Mirror.Builder(new UrlFetchTransport(), new JacksonFactory(), credential)
        .setApplicationName(Application.getAttribute(Application.APP_NAME)).build();
  }
  ...
  public static TimelineListResponse listItems(Credential credential, long count)
	...
	public static String findTimeLineItem(Credential credential, String setSourceItemId) throws IOException {
		Mirror.Timeline timelineItems = getMirror(credential).timeline();
		Mirror.Timeline.List list = timelineItems.list();
		list.setSourceItemId(setSourceItemId);
		TimelineListResponse response = list.execute();
		String id = "";
		
		if( response != null ) {
			List<TimelineItem> results = response.getItems();
